...
for i in range(l):
    ...
    while abs(p_1-p_0)> epsi:
        #if abs((g[i]-q[i]) < (g[i+1]-q[i+1])):
            p_1 = g[i] #Doesn't work. Only gives you one value of g. The other values are simply 0 because no other value is computed.
            p_0 = p_1
            list_of_p[i] = p_1
...
plt.plot(q,list_of_p,'r^',ms = 0.5, label = 'p')
